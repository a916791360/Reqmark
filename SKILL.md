...
description: 标注大师1.1.8 — 适用于根据 PRD 给多视图、动态渲染 HTML 原型添加需求标注，并在清单中编辑、删除或跟踪完成状态。
trigger: 标注|标注一下|开始标注|执行标注|标注大师|加标注|添加标注|annotation|执行标注大师
category: product
---

# 标注大师 1.1.8 (Annotation Master)

在 HTML 原型文件上添加可视化数字角标标注。**零侵入原布局**：仅添加 `data-am-target` 属性。**V1.1.8** 增加稳定编辑抽屉、动态目标重扫、前台页面自动识别和当前页连续编号。保留 scope 隔离、fixed 直定位、宽元素左置、滚动追踪、双击完成状态与编辑删除。
...
| V1.1.8 | **稳定编辑 + 动态页面完整标注**：抽屉内容不越界，textarea 输入/滚动时不再被重建；过滤监听 body 子树并自动重扫动态目标；识别登录、首页、查询结果、详情页并按当前页连续编号。 |
...
V1.1.8 要求每条前台标注写入准确 `page`：`login`、`home`、`search`、`detail`。运行时根据 `.view.active` 和 `list-view.yy-search-mode` 识别当前页面；后台原型继续使用 `scope`。角标、tooltip 与清单共同使用当前页面实际可见标注生成的连续编号。
...
4. `MutationObserver` — V1.1.8 过滤监听 `document.body` 子树；仅在新增/移除 `[data-am-target]` 或页面/scope 状态变化时重扫和重定位，并忽略标注系统自身 DOM，避免循环刷新
...
见模板文件 `templates/annotation.js` — 动态渲染引擎、前台 page/后台 scope 检测、动态目标过滤重扫、当前页连续编号、稳定编辑面板、智能视口夹紧、碰撞检测、滚动追踪与状态切换。
...
12. **动态页面回归（V1.1.8 必做）**：生成后实际走通登录页 → 首页 → 查询结果页 → 商品详情页，确认每页动态目标均显示角标，编号从 1 连续排列，清单标题与当前页面一致。
13. **编辑回归（V1.1.8 必做）**：打开清单编辑长文本，验证 textarea 可持续输入、独立滚动、保存/取消有效，且条目内容不超过抽屉宽度。
...
此 skill 已发布至 GitHub：`https://github.com/a916791360/Reqmark`
